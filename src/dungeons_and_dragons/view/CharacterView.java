...
public class CharacterView extends JFrame implements Observer, View {
	...
	public JComboBox<String> fighter_combobox;
	...
	private void initializeWindow() {
		...
		this.add(additem_label);
		// Object[] ob1 = new Object[] { null };
		// Object[] helmetTemp = this.items.stream().filter(p ->
		// p.getItem_type().equals(Game_constants.HELMET)).toArray();
		// Object[] helmet = Stream.of(ob1,
		// helmetTemp).flatMap(Stream::of).toArray();

		String[] fighter = new String[] { "Bully", "Nimble", "Tank" };
		fighter_combobox = new JComboBox<String>(fighter);
		// if (helmet.length > 0)
		// helmet_combobox.setRenderer(new ItemRenderer());
